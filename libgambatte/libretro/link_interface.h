/*
 * Link interface definitions for the frontend-hosted link bus.
 *
 * Not part of upstream libretro yet, and deliberately NOT patched into the
 * vendored libretro-common/include/libretro.h, which tracks libretro upstream
 * and should stay updatable. The block below is maintained byte-identically in
 * libretro-godot's src/LinkInterface.hpp and in mGBA's bundled
 * src/platform/libretro/libretro.h, so the three can be diffed directly.
 */
#ifndef GAMBATTE_LINK_INTERFACE_H
#define GAMBATTE_LINK_INTERFACE_H
